...
<!-- 快速导航栏部分 -->
	...
	<div class="content">
	<div class="find">
		<input onkeyup="this.value=this.value.replace(/\s+/g,'')" placeholder="输入UUID编号" type="text" id="qid" name="qid" style="width: 350px;margin-left: 40px;height: 50px;padding-left: 20px;transition: all 0.3s;
...
<br>
	...
	<div class="table" style="margin-top: 70px;width: 100%;">
  	...
  	总记录数<strong style="color: red">${pageBean.totalCount }</strong>条
    		...
    		<a href="${pageContext.request.contextPath}/admin_findByPage.action?currentPage=1&pageSize=${pageBean.pageSize }&cid=${cid}"><strong style="color: green">首页</strong></a>
    		<a href="${pageContext.request.contextPath}/admin_findByPage.action?currentPage=${pageBean.currentPage - 1}&pageSize=${pageBean.pageSize }&cid=${cid}"><strong style="color: green">上一页</strong></a>
			  		</c:if>
			  		<c:forEach begin="1" end="${pageBean.totalPage }" var="pagenum">
			  			...
			  			<c:if test="${pageBean.currentPage != pagenum }">
    			<a style="color: blue" href="${pageContext.request.contextPath}/admin_findByPage.action?currentPage=${pagenum}&pageSize=${pageBean.pageSize }&cid=${cid}">${pagenum}</a>
			  			</c:if>
			  		</c:forEach>
			  		<c:if test="${pageBean.currentPage !=pageBean.totalPage }">
			  			<a href="${pageContext.request.contextPath}/admin_findByPage.action?currentPage=${pageBean.currentPage + 1}&pageSize=${pageBean.pageSize }&cid=${cid}">
			  			<strong style="color: green">下一页</strong></a> | 
			  			<a href="${pageContext.request.contextPath}/admin_findByPage.action?currentPage=${pageBean.totalPage}&pageSize=${pageBean.pageSize }&cid=${cid}">
    		...
    		<script type="text/javascript">
    			function jump(){
    				...
    				//转向分页显示的Servlet
    				window.location.href="${pageContext.request.contextPath}/admin_findByPage.action?currentPage="+pagenum;
    			}
    		</script>
